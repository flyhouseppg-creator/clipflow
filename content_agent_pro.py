...
# cambia SOLO il MarginV. "bottom" conserva il valore storico (default invariato);
# le altre 4 salgono verso l'alto. Valori = frazione di PlayResY (1920); None = legacy.
SUBTITLE_POSITION_MV_FRAC = {
    "bottom": None,
    "three_quarter": 0.50,
    "middle": 0.61,
    "one_quarter": 0.76,
...
def validate_subtitle_position(pos: str) -> str:
    return pos if pos in SUBTITLE_POSITION_MV_FRAC else "bottom"
...
def generate_ass_file(words, output_path, is_portrait, font: str = "Arial", highlight_color: str = "#FFD700", text_color: str = "#FFFFFF", bg_style: str = "none", position: str = "bottom"):
    ...
    # "bottom" mantiene il valore legacy (mv calcolato sopra); le altre derivano da PlayResY=1920.
    position = validate_subtitle_position(position)
    pos_frac = SUBTITLE_POSITION_MV_FRAC.get(position)
    if pos_frac is not None:
        mv = round(1920 * pos_frac)
