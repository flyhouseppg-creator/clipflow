...
def run_job(job_id, video_path, audio_path, video_filename, user_prompt, selected_platforms, auto_zoom, subtitle_font: str = "Arial", subtitle_highlight_color: str = "#FFD700", subtitle_text_color: str = "#FFFFFF", subtitle_bg_style: str = "none"):
    def update(p, m):
        with jobs_lock:
...
async def start_job(background_tasks: BackgroundTasks, video: UploadFile = File(...), audio: Optional[UploadFile] = File(None), auto_zoom: str = Form("false"), platforms: List[str] = Form(...), subtitle_font: str = Form("Arial"), subtitle_highlight_color: str = Form("#FFD700"), subtitle_text_color: str = Form("#FFFFFF"), subtitle_bg_style: str = Form("none")):
    ...
    background_tasks.add_task(run_job, jid, vp, ap, video.filename, "", platforms, auto_zoom.lower()=="true", subtitle_font, subtitle_highlight_color, subtitle_text_color, subtitle_bg_style)
    return {"job_id": jid}
