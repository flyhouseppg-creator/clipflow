...
# Qualità video: scala la risoluzione di output mantenendo l'aspect ratio della
# piattaforma. Il valore = lato MINORE del frame (tutte le piattaforme hanno lato
# minore 1080, quindi 720p=down, 1080p=invariato, 4k=up).
QUALITY_MIN_SIDE = {"720p": 720, "1080p": 1080, "4k": 2160}

def validate_quality(q: str) -> str:
    return q if q in QUALITY_MIN_SIDE else "720p"
...
def build_video(input_path: Path, audio_path: Optional[Path], output_path: Path, platform_cfg: Dict, keep_segments: List[Dict], words: Optional[List[Dict]], max_sec: int, auto_zoom: bool, subtitle_font: str = DEFAULT_FONT, subtitle_highlight_color: str = "#FFD700", subtitle_text_color: str = "#FFFFFF", subtitle_bg_style: str = "none", subtitle_position: str = "bottom", quality: str = "720p", subtitles_enabled: bool = True, subtitle_karaoke: bool = True) -> None:
    ...
    scale = tier / min(w0, h0)
    w, h = round(w0 * scale / 2) * 2, round(h0 * scale / 2) * 2
    ...
    try:
        ...
        # Tutti gli -i PRIMA di qualsiasi opzione di output: ffmpeg recente rifiuta
        # i -map messi prima di un input. Niente clean_meta qui (i suoi -map
        # duplicherebbero gli stream): ogni caso mappa esplicitamente 1 video + audio.
        base_cmd = ["ffmpeg", "-y", "-i", str(cut_video)]
        if mp3_provided: base_cmd += ["-i", str(audio_path)]
        base_cmd += ["-map_metadata", "-1", "-tune", "zerolatency", "-movflags", "+faststart"]
        venc = ["-c:v", "libx264", "-preset", "superfast", "-crf", "23"]
        ...
        if mp3_provided and cut_has_audio:
            ...
            cmd = base_cmd + ["-filter_complex", f"{vf_filter};{amix}", "-map", "[vout]", "-map", "[aout]"] + venc + ["-c:a", "aac", "-b:a", "128k", str(final_video)]
        elif mp3_provided:
            ...
            cmd = base_cmd + ["-vf", vf, "-map", "0:v:0", "-map", "1:a:0"] + venc + audio_args + [str(final_video)]
        elif cut_has_audio:
            audio_args = ["-c:a", "copy"] if get_stream_codecs(cut_video).get("audio") == "aac" else ["-c:a", "aac", "-b:a", "128k"]
            cmd = base_cmd + ["-vf", vf, "-map", "0:v:0", "-map", "0:a:0?"] + venc + audio_args + [str(final_video)]
        else:
            cmd = base_cmd + ["-vf", vf, "-map", "0:v:0", "-an"] + venc + [str(final_video)]
...
.drop-icon{width:54px;height:54px;margin:0 auto 12px;border-radius:15px;background:radial-gradient(120% 100% at 50% 0%,#1c1810,#0b0906);border:1.5px solid rgba(240,180,41,.35);box-shadow:inset 0 0 0 1px rgba(0,0,0,.45),inset 0 1px 0 rgba(255,225,150,.12),0 8px 22px -10px rgba(240,180,41,.5);display:grid;place-items:center}
.drop-icon svg{width:26px;height:26px;display:block}
...
document.querySelectorAll("input[name='quality']").forEach(r=>r.addEventListener("change",()=>document.querySelectorAll(".qual").forEach(q=>q.classList.toggle("selected",q.querySelector("input").checked))));
;(function(){
    try{
